                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                        <h5 class="mb-0 fw-bold text-dark"></i>Detail Dokumen
                            #{{ $surat_keluar->nomor_surat }}</h5>
                        <span
                            class="badge bg-{{ $surat_keluar->status == 'selesai' ? 'success' : ($surat_keluar->status == 'revisi' ? 'warning text-dark' : ($surat_keluar->status == 'proses' ? 'primary' : 'secondary')) }} px-3 py-2 fw-bold">
                            {{ strtoupper($surat_keluar->status == 'proses' ? 'DIPROSES' : $surat_keluar->status) }}
                        </span>
                    </div>
                    <div class="card-body">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0" style="min-width: 1200px;">
                                ...
                                <tbody>
                                    @foreach ($surat_keluar->depositoItems as $item)
                                        <tr>
            ...
            <div class="col-lg-4">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body text-center py-5">
                        @if ($surat_keluar->status == 'proses')
                            <div class="spinner-grow text-primary mb-3" role="status"></div>
                            <h6>Dokumen sedang berada di:</h6>
                            <h4 class="text-primary fw-bold">
                                {{ str_replace('_', ' ', strtoupper($workflow->current_role ?? 'PINSI PELNAS')) }}
                            </h4>
                            <p class="text-muted small px-3">Menunggu verifikasi, persetujuan Tanda Tangan Elektronik
                                pejabat berwenang.</p>
                        @elseif($surat_keluar->status == 'revisi')
                        ...
                        @elseif($surat_keluar->status == 'selesai')
